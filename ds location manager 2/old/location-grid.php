...
<div class="ds-location-grid-section">
    ...
    <?php if (!empty($atts['heading'])) : ?>
        <div class="ds-location-grid-header">
            <?php 
            // Two-color heading
            $heading_parts = explode(' ', trim($atts['heading']));
            $split_position = intval($atts['heading_word_split']);
            
            if ($split_position > 0 && $split_position <= count($heading_parts)) {
                echo '<h2 class="ds-heading-split">';
                foreach ($heading_parts as $index => $word) {
                    $word_num = $index + 1;
                    if ($word_num == $split_position) {
                        echo '<span class="ds-heading-split__accent">' . esc_html($word) . '</span>';
                    } else {
                        echo '<span class="ds-heading-split__primary">' . esc_html($word) . '</span>';
                    }
