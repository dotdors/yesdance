...
class DS_Location_Manager_V2 {
    ...
    public function init() {
        ...
        add_filter('the_content', array($this, 'prepend_location_data_to_post'));
    }
    ...
    // Only handle location-related capabilities
    ...
    // Administrators get all location capabilities
    if (in_array('administrator', (array) $user->roles)) {
        return array('exist');
    }

    // Handle Location Manager capabilities
    if (in_array('ds_location_manager', (array) $user->roles)) {
        $user_location = get_user_meta($user_id, 'ds_assigned_location', true);
        
        // If they don't have an assigned location, deny all
        if (!$user_location) {
            return array('do_not_allow');
        }

        // Handle specific post capabilities (when $args[0] contains post ID)
        if (isset($args[0]) && is_numeric($args[0])) {
            $post_id = intval($args[0]);
            
            // Allow all capabilities for their assigned location
        ...
        // Handle general capabilities (no specific post ID)
        ...
        // Explicitly deny capabilities for other locations
        $deny_caps = array(
            'edit_others_locations',
            'delete_others_locations',
            'edit_private_locations', // if they shouldn't edit private
            'delete_private_locations'
        );
    ...
    if (in_array('ds_location_manager', (array) $user->roles) && $screen && $screen->post_type === 'ds_location') {
     ...
     // Method to adjust Location Manager caps
    public function setup_location_manager_caps() {
        $role = get_role('ds_location_manager');
        if ($role) {
            // Grant only what they need to edit/publish own Locations
...
/**
 * Ensure the core 'author' column appears for ds_location.
 */
...
public function location_sortable_columns($columns) {
    ...
    public function prepend_location_data_to_post($content) {
    if (!is_singular('post')) {
        return $content; // only for single posts
    }
    ...
    if (empty($terms) || is_wp_error($terms)) {
        return $content; // no location tag
    }

    // Assume one-to-one mapping → grab first term
    $term = $terms[0];

    // Find location post tied to this term
    ...
    if (empty($location)) {
        return $content; // no matching location
    }

    $location_id = $location[0]->ID;

    // Collect location meta
    $fields = array(
        'name'        => get_post_meta($location_id, '_ds_location_name', true),
    ...
    // Build display HTML
    $html = '<div class="ds-location-meta">';
    foreach ($fields as $label => $value) {
        if (!empty($value)) {
            $html .= '<p class="ds-location-' . esc_attr($label) . '"><strong>' 
                   . ucfirst($label) . ':</strong> ' . esc_html($value) . '</p>';
        }
    }
    $html .= '</div>';
