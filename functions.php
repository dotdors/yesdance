...
if ( ! isset( $content_width ) )
	$content_width = 1400; /* Default the embedded content width to 790px */
...
if ( ! function_exists( 'quark_setup' ) ) {
	function quark_setup() {
		...
		register_nav_menus( array(
				...
				'sitemap' => esc_html__( 'Sitemap Menu', 'quark' ),
				'sitemap2' => esc_html__( 'Sitemap2 Menu', 'quark' ),
				'sitemap3' => esc_html__( 'Sitemap3 Menu', 'quark' ),
				'sitemap4' => esc_html__( 'Sitemap4 Menu', 'quark' ),
				'sitemap5' => esc_html__( 'Sitemap5 Menu', 'quark' ),
				'sitemap6' => esc_html__( 'Sitemap6 Menu', 'quark' ),
				'newsletr' => esc_html__( 'newsletr Menu', 'quark' ),
				'topnav' => esc_html__( 'Top Menu', 'quark' )
			) );
...
*/
		...
		add_theme_support( 'custom-background', array(
				...
				'default-image' => trailingslashit( get_template_directory_uri() ) . 'images/subtlebkg.png'
			) );
...
//If you want to limit to just one category on the index page, uncomment this
//function my_home_category( $query ) { if ( $query->is_home() && $query->is_main_query() ) { $query->set( 'cat', '37'); } } add_action( 'pre_get_posts', 'my_home_category' );
 ...
 * @return void
 */
 add_image_size( 'mysquare', 450, 450, true );
...
function quark_fonts_url() {
	 ...
	 */
	$pt_sans = _x( 'off', 'PT Sans font: on or off', 'quark' );
	 ...
	 * Do not translate into your own language.
	 */
	$arvo = _x( 'off', 'Arvo font: on or off', 'quark' );
...
function quark_widgets_init() {
	...
	register_sidebar( array(
			...
			register_sidebar( array(
			'name' => esc_html__( 'Prefooter', 'quark' ),
			'id' => 'sidebar-prefooter',
			'description' => esc_html__( 'Bottom before the blue footer.  ', 'quark' ),
			'before_widget' => '<aside id="%1$s" class="urgent widget %2$s">',
			'after_widget' => '</aside>',
			'before_title' => '<h3 class="widget-title">',
			'after_title' => '</h3>'
		) );
			register_sidebar( array(
			'name' => esc_html__( 'Urgent Announcements', 'quark' ),
			'id' => 'sidebar-top',
			'description' => esc_html__( 'Only Shows when an announcement is present.  Drag a text widget here to use.', 'quark' ),
			'before_widget' => '<aside id="%1$s" class="urgent widget %2$s">',
			'after_widget' => '</aside>',
			'before_title' => '<h3 class="widget-title urgent">',
			'after_title' => '</h3>'
		) );
		register_sidebar( array(
			'name' => esc_html__( 'First News Page Widget Area', 'quark' ),
			'id' => 'sidebar-newspage1',
			'description' => esc_html__( '1 to 4 widget areas on the news page', 'quark' ),
			'before_widget' => '<aside id="%1$s" class="widget %2$s">',
			'after_widget' => '</aside>',
			'before_title' => '<h3 class="widget-title">',
			'after_title' => '</h3>'
	...
	register_sidebar( array(
			'name' => esc_html__( 'Second News Page Widget Area', 'quark' ),
			'id' => 'sidebar-newspage2',
			'description' => esc_html__( '1 to 4 widget areas on the news page', 'quark' ),
			'before_widget' => '<aside id="%1$s" class="widget %2$s">',
			'after_widget' => '</aside>',
			'before_title' => '<h3 class="widget-title">',
	...
	register_sidebar( array(
			'name' => esc_html__( 'Third News Page Widget Area', 'quark' ),
			'id' => 'sidebar-newspage3',
			'description' => esc_html__( '1 to 4 widget areas on the news page', 'quark' ),
			'before_widget' => '<aside id="%1$s" class="widget %2$s">',
			'after_widget' => '</aside>',
			'before_title' => '<h3 class="widget-title">',
	...
	register_sidebar( array(
			'name' => esc_html__( 'Fourth News Page Widget Area', 'quark' ),
			'id' => 'sidebar-newspage4',
			'description' => esc_html__( '1 to 4 widget areas on the news page', 'quark' ),
			'before_widget' => '<aside id="%1$s" class="widget %2$s">',
			'after_widget' => '</aside>',
			'before_title' => '<h3 class="widget-title">',
...
function quark_scripts_styles() {
	...
	wp_register_style( 'fontawesome', 'https://maxcdn.bootstrapcdn.com/font-awesome/4.5.0/css/font-awesome.min.css' , array( 'normalize' ), '4.5.0', 'all' );
...
if ( ! function_exists( 'quark_posted_on' ) ) {
	function quark_posted_on() {
		...
		$date = sprintf( '<i class="fa %1$s"></i> <a href="%2$s" title="Posted %3$s" rel="bookmark"><time class="entry-date" datetime="%4$s" itemprop="datePublished">Posted On %5$s</time></a>',
			$post_icon,
			esc_url( get_permalink() ),
			sprintf( esc_html__( '%1$s @ %2$s', 'quark' ), esc_html( get_the_date() ), esc_attr( get_the_time() ) ),
			esc_attr( get_the_date( 'c' ) ),
			esc_html( get_the_date('') ) // remove the F j(php date formatting) to show full date.
		);
	...
	$categories_list = '';
		...
		printf( wp_kses( __( '<div class="header-meta">%1$s<span class="post-categories">%3$s</span></div>', 'quark' ), array( 
			'div' => array ( 
				'class' => array() ), 
			'span' => array( 
				'class' => array() ) ) ),
...
function quark_continue_reading_link() {
	return '<p><a class="more-link" href="'. esc_url( get_permalink() ) . '" title="' . esc_html__( 'Continue reading', 'quark' ) . ' &lsquo;' . get_the_title() . '&rsquo;">' . wp_kses( __( 'Continue reading <span class="meta-nav">&rarr;</span>', 'quark' ), array( 'span' => array( 
			'class' => array() ) ) ) . '</a></p>';
}
...
if ( ! function_exists( 'quark_get_social_media' ) ) {
	function quark_get_social_media() {
		...
		$icons = array(
			...
			array( 'url' => of_get_option( 'social_bitbucket', '' ), 'icon' => 'fa-houzz', 'title' => esc_html__( 'Find me on Houzz', 'quark' ) ),
			...
			array( 'url' => of_get_option( 'social_email', '' ), 'icon' => 'fa-envelope', 'title' => esc_html__( 'Contact Us', 'quark' ) ),
...
function social_media_sc() {
		$output = '<div class="social-media-icons">';
		
		$output .= quark_get_social_media();
	$output .= '</div>';
...
add_shortcode("socials", "social_media_sc");	
/**
...
$linkColour = apply_filters( 'of_sanitize_color', of_get_option( 'link_color', '' ) );
	if ( !empty( $linkColour ) ) {
		$output .= "\na, .social-media-icons a,a:visited, h1, #site-info a, #site-info a:visited { ";
		$output .= "color: " . $linkColour . ";";
		$output .= "} #footercontainer .social-media-icons a,#footercontainer #site-info a, #footercontainer #site-info a:visited {color:white;}";
	}
	$linkColour2 = apply_filters( 'of_sanitize_color', of_get_option( 'link_color2', '' ) );
	if ( !empty( $linkColour2 ) ) {
		$output .= "\na:hover,.social-media-icons , #site-info a:hover, #site-info a:focus,a:hover, .main-navigation .current-menu-item > a:hover, .main-navigation .current-menu-ancestor > a:hover, .main-navigation .current_page_item > a:hover, .main-navigation .current_page_ancestor > a:hover, .main-navigation .current_page_parent > a:hover ,#site-navigation ul li a:hover,#site-navigation ul li a.active,#site-navigation ul li a.active:hover { ";
		$output .= "color: " . $linkColour2 . ";";
		$output .= " }";
	}
	
	$metaflag =  of_get_option( 'metaflag', '' ) ;
	if ( $metaflag == 'No' ) {
		$output .= "\n.header-meta { ";
		$output .= "display:none;";
		$output .= " }";
	}
$bkgflag =  of_get_option( 'bkgflag', '' ) ;
	if ( $bkgflag == 'Yes' ) {
		$output .= "\n#maincontentcontainer { ";
		$output .= "background:white;";
		$output .= " }";
	}
		if ( of_get_option( 'websitewidth', '' ) ) {
		$output .= "\n.site-size { ";
		$output .= "max-width: " . sanitize_text_field( of_get_option( 'websitewidth', '' ) ) . "px;";
		$output .= " }";
	}
			if ( of_get_option( 'sitepercent', '' ) ) {
		$output .= "\n.site-size { ";
		$output .= "width: " . sanitize_text_field( of_get_option( 'sitepercent', '' ) ) . "%;";
		$output .= " }";
	}
	if ( of_get_option( 'fontfam', '' ) ) {
		$output .= "\nh1 { ";
		$output .= "font-family: " . sanitize_text_field( of_get_option( 'fontfam', '' ) ) . ",Helvetica, Arial, sans-serif;";
		$output .= " }";
	}
		if ( of_get_option( 'fontfam1', '' ) ) {
		$output .= "\nh2,h3,h4,h5,h6,.nav-menu { ";
		$output .= "font-family: " . sanitize_text_field( of_get_option( 'fontfam1', '' ) ) . ",Helvetica, Arial, sans-serif;";
		$output .= " }";
	}
	if ( of_get_option( 'fontfam2', '' ) ) {
		$output .= "\nbody { ";
		$output .= "font-family: " . sanitize_text_field( of_get_option( 'fontfam2', '' ) ) . ",Helvetica, Arial, sans-serif;";
		$output .= " }";
	}
...
if ( ! function_exists( 'quark_before_woocommerce_wrapper' ) ) {
	function quark_before_woocommerce_wrapper() {
		echo '<div id="primary" class="site-size site-content row" role="main">';
	}
}
...
//for directory - makes user profile field un-editable by subsciber level
//also, removed more crap: http://isabelcastillo.com/hide-personal-options-wordpress-admin-profile
/*function wpse50730_script_enqueuer(){
    if(current_user_can('subscriber')) {
        echo '<script type="text/javascript">
        jQuery(document).ready( function($) {
            $(".form-table #cat_override").attr("readonly", true);
			 $(".form-table #member_since").attr("readonly", true);
...
function html5_search_form( $form ) { 
     $form = '<section class="search searchform"><form role="search" method="get" id="searchform" action="' . home_url( '/' ) . '" >
    <label class="screen-reader-text" for="s">' . __('',  'domain') . '</label>
     <input type="search" value="' . get_search_query() . '" name="s" id="s" placeholder="Start typing to search the site" />
     <input type="submit" id="searchsubmit" value="'. esc_attr__('Search The Site', 'domain') .'" />
     </form></section>';
     return $form;
}
 ...
 // wp-job-manager set default company image.
function new_default_image() {
    $myurl =  get_stylesheet_directory_uri(). '/images/company.png';
    return $myurl;
}
add_filter( 'job_manager_default_company_logo', 'new_default_image' ); // Where $priority is default 10, $accepted_args is default 1.
...
function tribe_alter_event_archive_titles ( $original_recipe_title, $depth ) {
	...
	$title_upcoming =   'Upcoming Events & Classes'; // List View: Upcoming events
	$title_past =       'Past Events & Classes'; // List view: Past events
	$title_range =      'Events & Classes for %1$s - %2$s'; // List view: range of dates being viewed
	$title_month =      'Events & Classes for %1$s'; // Month View, %1$s = the name of the month
	$title_day =        'Events & Classes for %1$s'; // Day View, %1$s = the day
	$title_all =        'All events & classes for %s'; // showing all recurrences of an event, %s = event title
	$title_week =       'Events & Classes for week of %s'; // Week view
