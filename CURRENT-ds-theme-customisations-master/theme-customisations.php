<?php
/**
 * Plugin Name:       DabbledStudios Theme Customisations
 * Description:       This is what makes your site special!

 * Version:           3.0.0
 * Author:            DabbledStudios
...
final class Theme_Customisations {
	...
	public function theme_customisations_css() {
		//wp_enqueue_style( 'custom-css', plugins_url( '/custom/style.min.css', __FILE__ ) );
       // wp_enqueue_style( 'custom-css', plugins_url( '/custom/styling.min.css', __FILE__ ) );
		 
		 wp_register_style(
	'plugin-styles',
	plugin_dir_url( __FILE__ ) .  'custom/dsstyle.css',
	array(),
	filemtime( plugin_dir_path( __FILE__ ) .  'custom/dsstyle.css' )
); 

wp_enqueue_style( 'plugin-styles' );
//wp_enqueue_style( 'plugin-styles-dominic' );
	}
	...
	public function theme_customisations_js() {
		wp_enqueue_script( 'custom-js', plugins_url( '/custom/custom.js', __FILE__ ), array( 'jquery' ),   filemtime( plugin_dir_path( __FILE__ ) .  'custom/customhome.js'));        if( is_front_page() ) {
                    wp_enqueue_script( 'custom-jshome', plugins_url( '/custom/customhome.js', __FILE__ ), array( 'jquery' ),  filemtime( plugin_dir_path( __FILE__ ) .  'custom/customhome.js') );
            }
	}
